...
export default function App() {
  ...
  const activeRoute = location.key === "task-detail"
    ? { key: "task-detail", label: "任务详情", icon: TerminalSquare }
  ...
  return <div className={cls("app-shell", sidebarCollapsed && "sidebar-collapsed")}>
    ...
    <aside className="sidebar"><button type="button" className="sidebar-toggle" title={sidebarCollapsed ? "展开侧栏" : "收起侧栏"} aria-label={sidebarCollapsed ? "展开侧栏" : "收起侧栏"} aria-expanded={!sidebarCollapsed} onClick={toggleSidebar}>{sidebarCollapsed ? <PanelLeftOpen size={15} /> : <PanelLeftClose size={15} />}</button><div className="brand"><div className="brand-mark">{BRAND.mark}</div><div className="brand-copy"><div className="brand-name">{BRAND.name}</div><div className="brand-subtitle">{BRAND.subtitle}</div></div></div><nav className="nav-list">{routes.map((route) => { const Icon = route.icon; const active = location.key === route.key || (location.key === "task-detail" && route.key === "tasks"); return <motion.button key={route.key} title={sidebarCollapsed ? route.label : undefined} aria-label={route.label} whileHover={sidebarCollapsed ? { scale: 1.04 } : { x: 3 }} whileTap={{ scale: 0.98 }} className={cls("nav-item", active && "active")} onClick={() => navigate(route.key)}><Icon size={18} /><span>{route.label}</span></motion.button>; })}</nav><div className="sidebar-foot"><div className="operator" title={sidebarCollapsed ? (user?.username || "operator") : undefined}><span className="status-dot" /><div><strong>{user?.username || "operator"}</strong><small>{user?.role || "single-user"}</small></div></div></div></aside>
    <main className="main"><header className="topbar"><div className="topbar-title"><div className="eyebrow"><ActiveIcon size={14} /> Control Surface</div><h1>{activeRoute.label}</h1></div><div className="top-actions"><div className="runtime-strip" aria-label="系统运行状态"><div className={cls("runtime-chip", health?.online ? "online" : "offline")} title="后端 API 状态"><Server size={14} /><span><small>API</small><strong>{health === null ? "检查中" : health.online ? "在线" : "离线"}</strong></span></div><div className={cls("runtime-chip", health?.mcp_enabled ? "online" : "inactive")} title="Model Context Protocol 状态"><PlugZap size={14} /><span><small>MCP</small><strong>{health === null ? "检查中" : health.mcp_enabled ? "已启用" : "未启用"}</strong></span></div><div className="runtime-chip" title="任务执行工作线程"><Cpu size={14} /><span><small>WORKERS</small><strong>{health?.task_workers ?? "-"}</strong></span></div><div className="runtime-chip system-clock" title="本地时间"><Clock3 size={14} /><span><small>LOCAL</small><strong>{clock.toLocaleTimeString("zh-CN", { hour12: false })}</strong></span></div></div><div className="topbar-command-group"><button className="icon-button topbar-icon-button" title="刷新页面和运行状态" aria-label="刷新页面和运行状态" onClick={() => { loadHealth(); window.location.reload(); }}><RefreshCw size={16} /></button><button className="icon-button topbar-icon-button" title="退出登录" aria-label="退出登录" onClick={logout}><LogOut size={16} /></button></div></div></header><RouteErrorBoundary routeKey={location.key}><Suspense fallback={<div className="route-loading"><span className="status-dot" />LOADING MODULE...</div>}><RouteComponent setPage={navigate} taskId={location.taskId} /></Suspense></RouteErrorBoundary></main>
  </div>;
}
