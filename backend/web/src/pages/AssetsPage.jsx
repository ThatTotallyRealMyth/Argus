...
function CatalogObservationsPanel({ detail, onOrigin }) {
  return <div className="catalog-workbench-pane"><section className="catalog-detail-section"><h4><Clock3 size={14} /> Sighting history</h4><p className="hint">A sighting is a historical asset snapshot from a scan. It does not necessarily represent a finding or a change.</p>
    {detail.observations?.length ? detail.observations.map((observation) => <div className="catalog-observation-row" key={observation.id}><div className="history-row"><Badge>{observation.source_type}</Badge><button className="link-button" onClick={() => onOrigin(observation.task_id, observation.origin_type)}>{originLabel(observation.origin_type, observation.task_id)}</button><time>{formatDate(observation.observed_at)}</time></div><details><summary>View snapshot data</summary><pre>{JSON.stringify(parseJSON(observation.payload, {}), null, 2)}</pre></details></div>) : <EmptyState text="No sightings recorded" />}
  </section></div>;
}
...
export default function AssetsPage({ setPage: navigate }) {
  ...
  const columns = {
    inventory: ["Selection", "Type", "Asset", "Risk", "Findings", "Changes", "Sightings", "Last seen"],
    ...
    <Panel title="Asset Browser" icon={<Network size={17} />}>
      ...
      <Modal open={Boolean(catalogDetail)}>{catalogDetail && <div className="library-editor catalog-detail">
        ...
        {catalogDetail.loading ? <div className="route-loading"><span className="status-dot" />Loading attack leads...</div> : <>
          <div className="catalog-detail-metrics"><div><span>Type</span><strong>{catalogDetail.asset?.kind}</strong></div><div><span>Risk</span><strong><Badge tone={riskTone(catalogDetail.asset?.risk_score)}>{riskLabel(catalogDetail.asset?.risk_score)} · {catalogDetail.asset?.risk_score}</Badge></strong></div><div><span>Findings</span><strong>{compactNumber(catalogDetail.asset?.vulnerability_count)}</strong></div><div><span>Changes</span><strong>{compactNumber(catalogDetail.asset?.change_count)}</strong></div><div><span>Sightings</span><strong>{compactNumber(catalogDetail.asset?.observation_count)}</strong></div><div><span>Last seen</span><strong>{formatDate(catalogDetail.asset?.last_seen_at)}</strong></div></div>
