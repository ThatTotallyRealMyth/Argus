...
export default function TrafficPage() {
  ...
  async function openDetail(id) {
    ...
    catch (error) { setSelected(null); setDetailError(`Failed to load details: ${error.message}`); }
  }

  return (
    <div className="stack">
      <Panel title="HTTP Response Filter" icon={<ListFilter size={17} />}>
        <AdvancedSearch value={search} fields={["url", "method", "status", "type", "source", "body", "request"]} placeholder="Search HTTP records, For example... status:200 && body:token && !url:logout" onApply={(value) => { setSearch(value); setPage(1); }} />
        <div className="filter-grid">
          <input id="http-status" name="http-status" placeholder="Status Code" value={filters.status} onChange={(e) => setFilters({ ...filters, status: e.target.value })} />
          ...
          <input id="http-body" name="http-body" placeholder="Text Keywords" value={filters.body} onChange={(e) => setFilters({ ...filters, body: e.target.value })} />
          <input id="http-min-length" name="http-min-length" placeholder="Minimum length" value={filters.min} onChange={(e) => setFilters({ ...filters, min: e.target.value })} />
          <input id="http-max-length" name="http-max-length" placeholder="Maximum length" value={filters.max} onChange={(e) => setFilters({ ...filters, max: e.target.value })} />
          <button type="button" className="ghost-button filter-reset" onClick={() => { setFilters({ status: "", contentType: "", body: "", min: "", max: "" }); setSelected(null); setDetailError(""); }}>Empty Filter</button>
        </div>
        {error && <div className="error-box">{error}</div>}
      </Panel>
      <div className="split-layout wide">
        <Panel title="Record List" icon={<FileSearch size={17} />}>
          <DataTable
            ...
            columns={["Status", "Length", "Type", "Source", "URL"]}
            ...
            empty="Not yet HTTP Records"
          />
          <Pager page={page} totalPages={data?.total_pages || 1} setPage={setPage} />
        </Panel>
        <Panel title="Details" icon={<Braces size={17} />}>
          {detailError ? <div className="error-box">{detailError}</div> : selected ? (
            <div className="detail">
              ...
              <h4>Response Body</h4>
              <pre>{selected.response_body || "No response body was saved; it may be binary or exceed the size limit."}</pre>
            </div>
          ) : <EmptyState text="Select a record to see the full response" />}
        </Panel>
      </div>
    </div>
  );
